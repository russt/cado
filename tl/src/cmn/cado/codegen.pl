...
#
# @(#)codegen.pl - ver 1.100 - 26-Sep-2013
#
...
#       Fix a bug in :hexencode, :urlencode ops - operators now encode newlines.
...
#  30-Apr-2011 (russt) [Version 1.92]
#       Allow {..}, [..], and (..) to quote here-now docs.  Allow multi-character sequences
...
#read input a line at a time, parse, and execute.
{
    ...
    while ($STRIPTOSHARPBANG && defined($line)) {
        ...
        chomp $line; $LINE_CNT = $$LINE_CNT_REF = ++$linecnt;
...
#printf STDERR "%s: line[%d] (%s)='%s'\n", "MAIN", $linecnt, $infile, $line;
            ...
            } elsif (&ifdefspec(\$line, \$linecnt, $use_stdin, $fhref, $infile, \$dointerpret)) {
                #this may force another loop
                printf STDERR "AFTER ifn?def: line='%s' dointerpret=%d\n", $line, $dointerpret if ($DEBUG);
#printf STDERR "%s: line[%d] (%s)='%s' dointerpret=%d\n", "IFDEF", $linecnt, $infile, $line, $dointerpret;

#NOTE:  we need to devise a mechanism to scan past a multi-line definition
#in the event the tested condition is false.

            } elsif (&ifspec(\$line, \$linecnt, $use_stdin, $fhref, $infile, \$dointerpret)) {
                #this may force another loop
...
sub discardHereNow
#if input has here-now starting quote, then just scan until we get to matching quote,
#discarding input.
{
    my ($line, $linecnt, $use_stdin, $fhref, $infile) = @_;

    #if we have a here-now doc...
    return 1 unless ($$line =~ /=\s*<<\s*[^<]/);

    my @tmp = split('<<', $$line);
    my $eoi_tok;
    if ($#tmp >= 0) {
        $eoi_tok = $tmp[$#tmp];
        $eoi_tok =~ s/\s//g;    #remove any white-space in eoi-tok
    } else {
    ...
    #special case - if eoi_tok is bracketing sequence, .. we require end quote to be correspond:
    $eoi_tok =~ s/\{/\}/g  if ( $eoi_tok =~ /^\{+$/ );
    $eoi_tok =~ s/\[/\]/g  if ( $eoi_tok =~ /^\[+$/ );
    $eoi_tok =~ s/\(/\)/g  if ( $eoi_tok =~ /^\(+$/ );
...
{
    my ($_line, $linecnt, $use_stdin, $fhref, $infile, $_doeval) = @_;
    my ($line) = ${$_line};
    ...
    $$_doeval = $doeval;    #set result parameter

    printf STDERR "%s: vardefined=%d doeval=%d\n", $token, $vardefined, $doeval if ($DEBUG);
#printf STDERR "%s: vardefined=%d doeval=%d\n", $token, $vardefined, $doeval;

    if ($doeval) {
        #otherwise, re-interpret the remainder of the line:
        ${$_line} = $def_expr;
#printf STDERR "IFDEF return line='%s'\n", $$_line;

        return 1;
    }

    &discardHereNow($_line, $linecnt, $use_stdin, $fhref, $infile);

    #we saw and ifdef statement and processed it:
    return 1;
}
...
#i.e., if string or variable evaluates to non-zero or non-empty, it is true
{
    ...
    #we saw an if statement and processed it:
    return 1;
}
...
#will create a variable $foo with contents 'blah'.
{
    ...
    #usage:  &definition(\$line, \$linecnt, $use_stdin, $fhref, $infile)
    my $linecnt = 1;
    &definition(\$assignment_txt, \$linecnt, 0, 0, 0, 0);
